...
function linearStiff!(du, u, p, t)
    du = λ*u
    return nothing
end

# Generate data with a stiff solver.
data_prob = ODEProblem(linearStiff!, u0, tspan)
...
p_clean = plot(X_obs, Y_obs[:, 1]; label="observed y1", lw=2)
...
plt = plot(
    ...
    layout=(3, 1),
    size=(900, 850),
    title="Van der Pol Neural ODE: baseNN"
)

display(plt)
savefig(plt, "vanderpol_collocation_baseNN2.png")
