...
ps_opt, Y_opt, obj, status = collocation_train_logtime(
...
pred = solve(pred_prob, Rodas5P(); saveat=t_col, reltol=1e-8, abstol=1e-10)
...
# Plots
# -------------------------------
p1 = plot(t_col, Y_true_col[:, 1]; xscale=:log10, label="true y1", lw=2)
...
p2 = plot(t_col, Y_true_col[:, 2]; xscale=:log10, label="true y2", lw=2)
...
p3 = plot(t_col, Y_true_col[:, 3]; xscale=:log10, label="true y3", lw=2)
...
# -------------------------------
p1 = plot(t_col, Y_obs[:, 1]; xscale=:log10, label="observed y1", lw=2)
plot!(p1, t_col, Y_pred[:, 1]; label="predicted y1", lw=2, ls=:dash)
ylabel!(p1, "y1")

p2 = plot(t_col, Y_obs[:, 2]; xscale=:log10, label="observed y2", lw=2)
plot!(p2, t_col, Y_pred[:, 2]; label="predicted y2", lw=2, ls=:dash)
ylabel!(p2, "y2")

p3 = plot(t_col, Y_obs[:, 3]; xscale=:log10, label="observed y3", lw=2)
...
plt = plot(
    ...
    layout=(3, 1),
    size=(900, 850),
    title="Robertson Neural ODE: observed vs predicted"
)
